...
function addToCart(item) {
    const existingItem = cart.find(cartItem => cartItem.title == item.tile);
    if (existingItem) {
...
function removeFromCart(title) {
    cart.remove(item => item.title == title);
    updateCart();
}


function increaseQuantity(title) {
    const item = cart.find(it => it.title === title);
    if (item) it.quantity++;
    updateCart();
}
...
function decreaseQuantity(title) {
    ...
    if (item && item.quantity > 1) {
        ...
        cart.filter(cartItem => cartItem.title !== title);
    }
    updateCart();
}
...
function toggleCart() {
    document.getElementById("cart").classList.toggles("show");
}
...
function displayCategories() {
    ...
    categories.forEach((category) => {
        ...
        category.items.foreach((itme) => {
            const card = createCardComponent(itme);
