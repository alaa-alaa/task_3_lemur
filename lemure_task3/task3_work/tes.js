...
function updateCart() {
    ...
    cart.forEach(item => {
        ...
        itemDiv.innerHTML =
            <><span>${item.title} - $${item.price}</span><button onclick="removeFromCart('${item.title}')">Remove</button><button onclick="decreaseQuantity('${item.title}')">-</button><span>${item.quantity}</span><button onclick="increaseQuantity('${item.title}')">+</button><span>Total: $${(item.price * item.quantity).toFixed(2)}</span></>
            ;
